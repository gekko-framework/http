...
class HttpMiddleware
{
    ...
    public function __invoke()
    {
        ...
        // If the route is null, the request is not routeable, treat it as an exception
        if (!$this->injector->getContainer()->has(Routing\Route::class)) {
            return $this->loadFromFileSystem($req, $resp);
        }
    ...
    public function loadFromFileSystem(IHttpRequest $httprequest, IHttpResponse $httpresponse)
    {
        // Get only the request path
        $request = $httprequest->getURI()->getPath();

        $file = $httprequest->getRootDir($request);
        if (file_exists($file)) {
            ...
            if (!$isDirectory) {
                ...
                $content_type = $this->getContentTypeByExt($ext);
            }

            if (empty($content_type) && \class_exists('\finfo')) {
                $finfo =  new \finfo(FILEINFO_MIME);
                $content_type = $finfo->file($file);
            }

            // Fallback
            if (empty($content_type) && \function_exists('mime_content_type') && isset(self::$content_types[mime_content_type($file)])) {
            ...
            $httpresponse->setBody(ob_get_clean());
            return $httpresponse;
        } else {
            $httpresponse->setHeaderLine("HTTP/1.1 404 Resource not found");
            $httpresponse->setBody("Resource not found");
            return $httpresponse;
        }
    }
    ...
    private static $content_types = array(
            // Applications
            "application/atom+xml", "application/ecmascript", "application/javascript", "application/EDI-X12",
            "application/EDIFACT", "application/json", "application/javascript", "application/ecmascript",
            "application/octet-stream", "application/ogg", "application/pdf", "application/postscript", "application/rdf+xml",
            "application/rss+xml", "application/soap+xml", "application/font-woff", "application/x-font-woff",
            "application/xhtml+xml", "application/xml", "application/xml-dtd", "application/xop+xml", "application/zip",
            "application/gzip",

            // Audio
            "audio/basic",  "audio/L24",    "audio/mp4",    "audio/mpeg",   "audio/ogg",    "audio/vorbis", "audio/vnd.rn-realaudio",
            "audio/vnd.wave",   "audio/webm",

            // Images
            "image/gif", "image/jpeg", "image/pjpeg", "image/png", "image/svg+xml", "image/tiff",

            // Model
            "model/iges", "model/mesh", "model/vrml", "model/x3d+binary", "model/x3d+fastinfoset", "model/x3d+binary",
            "model/x3d-vrml", "model/x3d+vrml", "model/x3d+xml",

            // Multipart
            "multipart/mixed", "multipart/alternative", "multipart/related", "multipart/form-data", "multipart/signed",
            "multipart/encrypted",

            // Text
            "text/cmd", "text/css", "text/csv", "text/html", "text/javascript", "application/javascript", "text/javascript",
            "application/javascript", "text/plain", "text/vcard", "text/xml",

            // Video
            "video/mpeg", "video/mp4", "video/ogg", "video/quicktime", "video/webm", "video/x-matroska", "video/x-ms-wmv",
            "video/x-flv",
            ...
            "application/vnd.google-earth.kml+xml", "application/vnd.google-earth.kmz", "application/vnd.dart",
            "application/vnd.android.package-archive", "application/vnd.ms-xpsdocument",

            // Prefix x
            "application/x-7z-compressed", "application/x-deb", "application/x-dvi", "application/x-font-ttf",
            "application/x-javascript", "application/x-latex", "application/x-mpegURL", "application/x-rar-compressed",
            "application/x-shockwave-flash", "application/x-stuffit", "application/x-tar", "application/x-www-form-urlencoded",
